...
//	void Dibujar(Graphics^ g, int x, int y, int ancho, int largo);
	void DibujarNave(Graphics^ g,char identificar);
...
void Nave::Sprite_Enemigo() 
	{ 
		sprite = GetSprite(this->tipo, this->bando); 
	}

void Nave::DibujarNave(Graphics^ g,char identificar)
{
	if (estado == EstadoNave::Vivo)
	{
		Bitmap^ img = gcnew Bitmap(gcnew String(sprite.c_str()));
		img->MakeTransparent(img->GetPixel(0, 0));
		identificar=='E' ? g->DrawImage(img, xSprite, ySprite + 50, ancho, -largo): g->DrawImage(img, xSprite, ySprite + 50, ancho, largo);
